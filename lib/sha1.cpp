...
int SHA1_Init(SHA1_CTX *c)
{
    ...
    qe_cleanse(c, sizeof(SHA1_CTX)); /* Security consideration */
    //memset(c, 0, sizeof(SHA1_CTX));
...
static int SHA1_ProcessBlock(SHA1_CTX *ctx, const void *block)
{
    uint32_t t;
    ...
    SHA1_PrepareScheduleWord(static_cast<const uint32_t*>(block), W);
