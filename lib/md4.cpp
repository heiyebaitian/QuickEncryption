...
/*
 * QE_MD4_F/QE_MD4_G/QE_MD4_H definition, refer rfc1320, section 3.4
 */

/*
 * Condition
 * In each bit position, QE_MD4_F acts as a conditional:
 *   if X then Y else Z.
 */
static uint32_t QE_MD4_F(uint32_t x, uint32_t y, uint32_t z)
{
    return (x & y) | ((~x) & z);
}
...
/*
 * Majority
 * In each bit position, QE_MD4_G acts as a majority function:
 *   if at least two of X, Y, Z are on, then QE_MD4_G has a "1" bit in that bit position, else QE_MD4_G has a "0" bit.
 */
static uint32_t QE_MD4_G(uint32_t x, uint32_t y, uint32_t z)
{
    return (x & y) | (x & z) | (y & z);
}
...
/*
 * Parity
 * QE_MD4_H is the bit-wise XOR or "parity" function
 */
static uint32_t QE_MD4_H(uint32_t x, uint32_t y, uint32_t z)
{
    return x ^ y ^ z;
}
...
static md4_func g[3] =
{
    QE_MD4_F,  /*  0 ~ 15 operations */
    QE_MD4_G,  /* 16 ~ 31 operations */
    QE_MD4_H   /* 32 ~ 47 operations */
};
